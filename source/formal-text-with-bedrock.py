...
def get_formal_text_stream(text, model_id=None, max_tokens=4096, temperature=1.0, config_manager=None):
    """
    Generate formal text paraphrase using Claude model.
    
    Args:
        text: Text to paraphrase formally
...
def main():
    """Main function for formal text paraphrasing CLI."""
    # Initialize configuration manager
    config = default_config
    
    parser = argparse.ArgumentParser(description="Paraphrase text in a formal and logical way using Claude AI model.")
    parser.add_argument("text", nargs="?", help="Text to paraphrase")
    parser.add_argument("-f", "--file", help="Input file containing text to paraphrase")
    parser.add_argument("-o", "--output", help="Output file to save the formal text")
    ...
    # Process formal text paraphrasing with streaming output
    formal_text = ''
    print(OUTPUT_COLOR_START, end="")  # Set text color to white with black background
    try:
        for text_chunk in get_formal_text_stream(text, args.model, args.max_tokens, args.temperature, config):
            print(text_chunk, end="", flush=True)
            formal_text += text_chunk
    ...
    if args.output:
        try:
            with open(args.output, 'w', encoding='utf-8') as file:
                file.write(formal_text)
            print(f"\nFormal text saved to {args.output}")
        except IOError:
