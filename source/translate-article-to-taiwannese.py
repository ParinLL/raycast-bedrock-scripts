...
def get_translation_stream(text, model_id=None, max_tokens=4096, temperature=1.0, config_manager=None):
    """
    Translate text to Traditional Chinese using Claude model with streaming.
...
def main():
    """Main function for translation CLI."""
    # Initialize configuration manager
    config = default_config
    
    parser = argparse.ArgumentParser(description="Translate text to Traditional Chinese using Claude AI model.")
    parser.add_argument("text", nargs="?", help="Text to translate")
    parser.add_argument("-f", "--file", help="Input file containing text to translate")
    parser.add_argument("-o", "--output", help="Output file to save the translation")
    ...
    # Process translation with streaming output
    translation = ''
    print(OUTPUT_COLOR_START, end="")  # Set text color to white with black background
    try:
        for text_chunk in get_translation_stream(text, args.model, args.max_tokens, args.temperature, config):
            print(text_chunk, end="", flush=True)
            translation += text_chunk
